 ...
 */
namespace Hot_Blocks\Asset_Loader;
...
function enqueue_assets( $manifest_path, $opts = [] ) {
	...
	if ( empty( $assets ) ) {
		// Trust the theme or plugin to handle its own asset loading.
		return false;
	}
	...
	// There should only be one JS and two CSS file emitted per plugin or theme.
